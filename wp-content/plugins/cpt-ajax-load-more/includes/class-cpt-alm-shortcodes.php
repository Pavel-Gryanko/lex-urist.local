...
class CPT_ALM_Shortcodes {
	...
	public static function generate_shortcode($params = array()) {
		...
		$atts = shortcode_atts( apply_filters('cpt_alm_shortcode_defaults',array(
			...
			'scroll' => false,
			'grid' => '1',
			'item_class' => 'cpt-alm-item',
			'container_class' => 'cpt-alm-wrapper',
	...
	public static function fire_shorcode(){
		...
		$containerClass = (!empty(self::$attributes['container_class'])) ? sanitize_html_class(self::$attributes['container_class']) : 'cpt-alm-wrapper';

		$grid = (!empty(self::$attributes['grid'])) ? self::$attributes['grid'] : 1;
   		$gridClass = 'cpt-alm-grid-'.$grid;	
   		$itemClass = !empty(self::$attributes['item_class']) ? sanitize_html_class(self::$attributes['item_class']) : 'cpt-alm-item';
		$itemClass = $itemClass.' cpt-alm-col-'.(12 / $grid);
		...
		if ( $the_query->have_posts() ) {
			...
			$html .= '<div class="'.$containerClass.' '.$gridClass.'" '.$data_atts.'>';
			$cpt_alm_curr_item = 1;
			$cpt_alm_curr_page = 1;
